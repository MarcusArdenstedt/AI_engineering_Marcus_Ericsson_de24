...
from backend.constants import VECTOR_DATABASE
import lancedb
...
def retrieve_top_documents(query: str, k=3) -> str:
    """ 
    Use vector search to find the closest k matching documents to the query
    """
    result = vector_db["articles"].search(query=query).limit(k).to_list()
    top_result = result[0]
    return f"""
